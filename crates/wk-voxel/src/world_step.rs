...
pub fn step_world(
...

    {
        ...
        if let (Some(support_map), Some(landscape_store)) = (support.as_mut(), landscape.as_mut()) {
            ...
            if landscape_busy || !dirty.is_empty() || support_due {
                // Seed walk exact-skips !has_competent — rain-dirty
                // ocean/sky is leftover. Occupancy is the source of truth.
...

    {
        ...
        apply_leftover_motor(world, temperature, cfg.steam, Some(humidity));
        if let (true, Some(t0), Some(t)) = (profile, t0, timings.as_mut()) {
