...
fn throughput_widens_a_conduit_and_conserves_mineral() {
    ...
    // flow through it stays as it was. Total mineral must not move.
    let mut w = World::new(31);
    w.ensure_chunk(ChunkCoord::new(0, 0));
    for x in 0..24 {
        w.set_cell(x, 0, Cell::solid(MaterialId::Bedrock));
    }
    // Two identical limestone columns. Only the left one is fed.
    let lime = |pore: u8| {
    ...
    for y in 1..=10 {
        w.set_cell(5, y, lime(140));
        w.set_cell(15, y, lime(140));
    }
    // Bedrock walls must reach *above* the fill line, or the free surface
    // spreads sideways along the open row and feeds both columns.
    for y in 1..=13 {
        for x in [4, 6, 14, 16] {
            w.set_cell(x, y, Cell::solid(MaterialId::Bedrock));
        }
    }
    // Widest aperture anywhere in a column — the conduit, wherever it forms.
    let widest = |w: &World, x: i32| -> u8 {
        (1..=10)
            .filter_map(|y| w.get_cell(x, y))
            .filter(|c| c.material == MaterialId::Limestone)
            .map(|c| c.pore)
            .max()
            .unwrap_or(255) // fully dissolved counts as maximally open
    };
    let fed_before = widest(&w, 5);
    let dry_before = widest(&w, 15);
    let mineral_before = crate::audit::mineral_total(&w);

    // Keep the left column topped up so water keeps moving through it.
    for _ in 0..900 {
        w.set_cell(5, 11, Cell::water());
        tick(&mut w);
    }

    let fed_after = widest(&w, 5);
    let dry_after = widest(&w, 15);
    assert!(
        fed_after > fed_before,
        "throughput should open a conduit along the flow path ({fed_before} -> {fed_after})"
    );
    assert_eq!(
        dry_after, dry_before,
        "a column with no flow must not erode"
    );
    // Water was added every tick, so mineral is the invariant here, not water.
    assert_eq!(
        crate::audit::mineral_total(&w),
        mineral_before,
        "aperture growth must only move mineral, never create or destroy it"
    );
}
