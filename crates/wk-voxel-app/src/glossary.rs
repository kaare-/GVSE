...
const KEYS: &[(&str, LineKind)] = &[
    ...
    ("C        condensation — falling rain, or a snowflake below freeze (default on)", LineKind::Body),
    ...
    ("Tab→Climate→Cavity humidity   master switch for the steam pipe", LineKind::Body),
    ("         (play default on). Cadence / leftover-field knobs in that", LineKind::Dim),
    ("         panel stay inert while the pipe is on.", LineKind::Dim),
    ("", LineKind::Blank),
    ("Overlays", LineKind::Head),
    ("H        humidity tile raster (default on; Tab: resample + min mass)", LineKind::Body),
    ("         also washes sealed-cave humidity. Steam haze draws separately", LineKind::Dim),
    ("         when cavity humidity is on — it is not sky rain.", LineKind::Dim),
    ...
    ("P        steam pipe — dim claimed boiler, mid straw, bright mouth,", LineKind::Body),
    ("         brightest live puff. Leftover-field volume only if the pipe is off.", LineKind::Dim),
    ...
    ("O        toggle living creatures on / off (not a single step)", LineKind::Body),
    ("", LineKind::Blank),
    ...
    ("", LineKind::Blank),
    ("F2 only — these shadow the global keys while the editor is open", LineKind::Head),
    ("1–8      module brushes (nucleus … symbiont)", LineKind::Body),
    ("A / T / W / F   Atom / plant / seaweed / fungus templates", LineKind::Body),
    ("E / P    erase / paint     Enter then click to plant", LineKind::Body),
    (", / .    symbiont water    − / = symbiont energy", LineKind::Body),
    ("S / L    save / load a blueprint", LineKind::Body),
];
...
const WATER: &[(&str, LineKind)] = &[
    ...
    ("Condensation (C) spends humidity on falling rain or a snowflake.", LineKind::Body),
    ("", LineKind::Blank),
    ...
    ("that were not in the ocean or the air. C already pays from", LineKind::Body),
    ("humidity. There is no second cloud store and no sky faucet.", LineKind::Body),
    ...
    ("Pore water seeps sideways and down. It never freefalls through", LineKind::Body),
    ("solids. The steam pipe also wicks and pumps it upward along a", LineKind::Body),
    ("straw — that is not freefall. Standing pond sides and beds soak", LineKind::Body),
    ("banks at full permeability; thin films still shed on dry ground.", LineKind::Body),
    ("A pond overflows its crest only when the source cell is already", LineKind::Body),
    ...
    ("", LineKind::Blank),
    ("Words", LineKind::Head),
    ("sat        water units in one cell (255 = a full Air cell)", LineKind::Body),
    ...
    ("straw      locked pipe path from a hot reservoir to a mouth", LineKind::Body),
    ("main       straw that vents to open air or an open cave", LineKind::Body),
    ("feeder     buried straw that joins a main instead of surfacing", LineKind::Body),
    ("mouth      Air the atmosphere can reach — sky or an open cave", LineKind::Body),
    ("sinter     pale Flowstone dropped at a vent from dissolved load", LineKind::Body),
];
...
const SKY: &[(&str, LineKind)] = &[
    ...
    ("3. C  leftover vapor condenses. Warm air rains a falling drop.", LineKind::Body),
    ("      Below freeze it gathers a snowflake — never liquid rain.", LineKind::Body),
    ("4. H  paints sky humidity plus sealed-cave humidity (same wash).", LineKind::Body),
    ("      Steam / cavity haze draws separately when cavity humidity is on.", LineKind::Body),
    ("      Shade and drizzle read the sky field. There is no N bank layer.", LineKind::Body),
    ...
    ("", LineKind::Blank),
    ("Words", LineKind::Head),
    ("drizzle    C condensation (HUD tag). Falling rain when warm.", LineKind::Body),
    ("snow       C / surplus below freeze — a full-cell flake, not a faucet", LineKind::Body),
    ("haze       H overlay: sky vapour + sealed cave air (same soft white)", LineKind::Body),
    ("deck       retired word — vapour rise is lapse-driven, not a height lock", LineKind::Dim),
...
const GROUND: &[(&str, LineKind)] = &[
    ...
    ("mats float, waterlog, and bind to roots / mycelium (the cream field).", LineKind::Body),
    ("Roof collapse drops wide Air ceilings. Shear / compaction are", LineKind::Body),
    ("opt-in on Tab → Physics.", LineKind::Body),
    ("", LineKind::Blank),
    ("Hot springs / geysers", LineKind::Head),
    ("A wet hot hill grows one straw to the surface (or an open cave).", LineKind::Body),
    ("Nearby boilers join as feeders if they are closer than their own", LineKind::Body),
    ("sky. Flow erodes pores (Tab → Climate → pipe erosion gain); the", LineKind::Body),
    ("dissolved load rides the water out and drops sinter at the mouth.", LineKind::Body),
    ("The main simmers seven beats, then erupts one — a jet, not a puff.", LineKind::Body),
    ("", LineKind::Blank),
...
const LIFE: &[(&str, LineKind)] = &[
    ...
    ("Roots drink cell sat. Leaves take sky light (humidity shades).", LineKind::Body),
...
const HUD: &[(&str, LineKind)] = &[
    ...
    ("steam     off, or  Nc P=mains+feeders/cells sat=<mass> u=<live>", LineKind::Body),
    ("          Nc = cavity-seat count. sat = pipe water mass. u = live lumen.", LineKind::Dim),
    ("cave_h    sealed ambient cave humidity cells (not the steam= field)", LineKind::Body),
    ...
    ("land      detached rigid landscape bodies (floating / sliding rock)", LineKind::Body),
    ("creatures living / cap   (p plants  f fungi  a atoms)", LineKind::Body),
    ("dead      lingering corpses", LineKind::Body),
    ("", LineKind::Blank),
    ("Click a cell: sat is fill of that material's capacity, not always /255.", LineKind::Body),
    ("dissolved mineral rides liquid; suspended silt drops when flow stops.", LineKind::Body),
    ("Inspector leftover= is leftover-field volume. With the pipe on it", LineKind::Body),
    ("prints pipe= instead — the P overlay band (boiler / straw / mouth / live).", LineKind::Body),
    ("", LineKind::Blank),
    ...
    ("If hum is huge and FPS dies, the sky store is over-full. C still", LineKind::Body),
    ("lotteries every over-sat tile. Start a new world (R) after a flood.", LineKind::Dim),
];
