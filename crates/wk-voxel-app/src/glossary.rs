...
const KEYS: &[(&str, LineKind)] = &[
    ...
    ("Tab→Climate→Cavity humidity   boil free/pore water; soft 4×4 haze (not sky H)", LineKind::Body),
    ("", LineKind::Blank),
    ...
    ("W        F2 only — seaweed stamp, not a weather faucet", LineKind::Dim),
    ("", LineKind::Blank),
...
const HUD: &[(&str, LineKind)] = &[
    ...
    ("cavity_h  pressurized cavity humidity (wire: steam); soft haze; never writes sky H", LineKind::Body),
