...
const KEYS: &[(&str, LineKind)] = &[
    ...
    ("H        humidity tile raster + wind streaks (default on)", LineKind::Body),
...
const WATER: &[(&str, LineKind)] = &[
    ...
    ("Dry porous faces take a material-rate soak each tick; leftover", LineKind::Body),
    ("overflows the crest only when the source cell is already at that", LineKind::Body),
    ("height — ponds do not invent head to climb hillsides.", LineKind::Body),
