...
const KEYS: &[(&str, LineKind)] = &[
    ...
    ("N        soft echo of wet humidity tiles (default off)", LineKind::Body),
...
const SKY: &[(&str, LineKind)] = &[
    ...
    ("4. N  optional picture of wet humidity tiles (default off).", LineKind::Body),
    ("      Banks sit on those tiles. They do not rain, store, or lock to a deck.", LineKind::Body),
    ("", LineKind::Blank),
    ...
    ("nimbus     how many N echo parcels are drawn (0 if N is off)", LineKind::Body),
    ("echo       display mass of those parcels — not a water store", LineKind::Body),
    ("parcel     one soft N bank (picture only — rain is real water)", LineKind::Body),
