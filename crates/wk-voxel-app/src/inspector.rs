...
use wk_voxel::{
    cave_vapour_pressure_norm, is_fungus, is_land_plant, permeability_cell, soft_litter_at,
    steam_at, void_is_confined, water_capacity_cell, Atom, Cell, Corpse, GeotechMap, Humidity,
...
pub fn draw_block_inspector(
    ...
    match cell {
        Some(c) => {
            ...
            if steam > 0
                || (c.material == MaterialId::Air && void_is_confined(world, gx, gy))
                || (c.material == MaterialId::Air && c.sat.0 > 0 && temp_c >= 95.0)
            {
                ...
                let press = cave_vapour_pressure_norm(world, humidity, temperature, gx, gy);
                lines.push(format!(
                    "steam={steam}/255  confined={}  pressure={press:.2} (cave vapour = H under pressure)",
