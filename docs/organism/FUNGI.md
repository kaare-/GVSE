...
- **Symbiont treaty** — opt-in `ModuleId::Symbiont` (`0x16`, mint) on both
  ...
  or sugar moved) do not consume the plant's partner slot — the scan keeps
  looking for a cream that can trade. Each plant gets at most one successful
  cream partner per pulse (tick-rotated atom order) so mature multi-root
  editor plantings cannot starve later spore / sprout clones of the contact
  budget. **Strain↔strain**
