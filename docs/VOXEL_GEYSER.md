...
| Pore pressure transport | Phase expansion reverse-seeps ~10 hops by default, widens rock along the path, and vents dissolved load as artesian sinter. Drive is force (not minted mass); conduits stay rock (`mint_void = false`). |
| Pressure HUD | Inspector shows `pressure=` on hot saturated rock (pore flash) and on cavity vapour seats. Overlay **P** paints the pore/cavity pressure gradient (indigo → magenta → amber). Not a continuum PDE — wetness × superheat blended with nearby cavity fill. |
