...
| Key | What |
|-----|------|
| **N** | Soft cloud banks as a **static humidity echo** (no drift / scroll animation). Far/mid/front depth layers default off for FPS; Tab can raise them. |
...
| Signal | Visual |
|--------|--------|
| Humidity → parcels | Soft active banks (`N`) derived from wet seats each tick — **no wind scroll** |
...
| Wind | `V` streaks from local field; clouds do not fake-scroll on climate wind |
