...
`H` / `V` skip paint for tiles that cannot touch the viewport
(probe the camera tile box when it is smaller than the vapour map);
the sim field still runs at full rate off-screen (ring + pan).
