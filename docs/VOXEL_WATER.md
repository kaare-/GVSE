...
- **Organic** is contextual: grounded beach litter and waterlogged/sunk mats scour under flow (deposits stay `WATERLOGGED` so they remain bedload). Thick / mycelium-bound floating rafts stay wind-owned. Thin unbound floating film may scour under cascade so shore rings do not seal water into sticky bubbles; stream drift uses per-column push plus real `flow_bias` under the mat (Organic rides the same current as water) and may wash thin film over cascade lips.
