...
- **Settle:** After seepage, every tick runs `wake_unsupported_grains` + `wake_unstable_slopes` then multi-pass fall and multi-pass repose (up to `GRAIN_SETTLE_PASSES`). Fall alone left Organic/sand as vertical cliff faces; repose now keeps avalanching until the pile is flat (max_step ≈ 0). **Snow / Ice / Organic** float only on **grounded** full standing water; suspended mid-air full-sat is not a seat.
