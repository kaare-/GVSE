...
- **Settle:** After seepage, unsupported grains run **multi-pass** fall + repose (up to `GRAIN_SETTLE_PASSES`, default 384) so sky litter seats in one tick after unpause. F3 pauses the full CA while open; on close (and Space unpause) `wake_unsupported_grains` re-dirties floating sand/Organic whose dirty wake was lost, then the next tick seats them. A quiet-tick throttle also re-scans every 15 ticks.
