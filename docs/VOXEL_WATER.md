...
- **Settle:** After seepage, every tick runs `wake_unsupported_grains` then multi-pass fall + repose (up to `GRAIN_SETTLE_PASSES`). This matters when lakes/rain keep a dirty plan that is far from F3 paint — without the wake, floating sand only dripped via roof collapse. **Snow / Ice / Organic** float only on **grounded** full standing water (column reaches solid); suspended mid-air full-sat (often invisible) is not a seat.
